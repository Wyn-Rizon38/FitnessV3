...
// Initialize defaults so we never call count() on null
$member_name = 'Member';
$member_id = null;
$schedules = [];
$joined = [];
...
if (!empty($schedules) && !empty($joined)) {
    foreach ($schedules as $sched) {
        ...
        if ($schedId && in_array($schedId, $joined, true)) {
            ...
            if (isset($sched['day_of_week'], $sched['start_time'], $sched['end_time'])
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-dark table-striped text-center align-middle">
                ...
                <?php if (empty($schedules)): ?>
                    <tr><td colspan="5" class="text-center text-muted">No classes scheduled.</td></tr>
                <?php else: ?>
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?= htmlspecialchars(trim("$start to $end")) ?></td>
                            <td>
                                <div class="d-flex flex-wrap gap-1 justify-content-center">
                                    <?php if ($schedId && in_array($schedId, $joined, true)): ?>
                                        <a href="schedule.php?unjoin=<?= $schedId ?>" class="btn btn-danger btn-sm">Unjoin</a>
                                    <?php else: ?>
                                    ...
                                    <button class="btn btn-info btn-sm" type="button" data-bs-toggle="collapse" data-bs-target="#members-<?= $schedId ?>">
                                        Show Members
                                    </button>
                                </div>
                            </td>
                        ...
                        <tr class="collapse" id="members-<?= $schedId ?>">
                            <td colspan="5" class="bg-dark text-light">
                                <strong>Members Joined:</strong>
                                <ul class="mb-0">
                                <?php
                                if ($schedId) {
                                    $memStmt = $conn->prepare("SELECT m.fullname FROM class_members cm JOIN members m ON cm.member_id = m.user_id WHERE cm.class_id = ?");
                                    if ($memStmt) {
                                        $memStmt->bind_param("i", $schedId);
                                        $memStmt->execute();
                                        ...
                                        echo '<li class="text-muted">Unable to load members.</li>';
                                    }
                                } else {
                                    echo '<li class="text-muted">Invalid class id.</li>';
                                }
                                ?>
                                </ul>
                            </td>
